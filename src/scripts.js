...
document.addEventListener('DOMContentLoaded', function() {
 ...
 document.getElementById('simulacaoForm').addEventListener('submit', function(e) {
     ...
     const valorParcela = document.getElementById('valorParcela').value || 'A combinar';
     
     // Format the message
     const message = `Olá! Gostaria de simular um crédito com os seguintes dados:%0A%0A` +
                   `*Nome:* ${nome}%0A` +
                   `*Telefone:* ${telefone}%0A` +
                   `*Valor do Crédito:* R$ ${parseFloat(valorCredito).toLocaleString('pt-BR', {minimumFractionDigits: 2})}%0A` +
                   `*Entrada:* R$ ${parseFloat(entrada).toLocaleString('pt-BR', {minimumFractionDigits: 2})}%0A` +
                   `*Modalidade:* ${modalidade}%0A` +
                   `*Valor da Parcela:* ${valorParcela === 'A combinar' ? 'A combinar' : 'R$ ' + parseFloat(valorParcela).toLocaleString('pt-BR', {minimumFractionDigits: 2})}`;
